...
public:
    ...
    std::string find_matching_plugin(const std::string& topic_name, const std::string& msg_type);

    /// Render a topic message using a matched Python plugin module.
    /// Converts input json_body string -> Python dict msg, calls render(msg, state),
    /// updates state dict, and returns UI specification as nlohmann::json.
    nlohmann::json render_message(const std::string& module_name,
                                  const std::string& topic_name,
                                  const std::string& json_body);

    /// Get list of loaded Python plugins
    const std::vector<PythonPluginInfo>& loaded_plugins() const { return loaded_plugins_; }
