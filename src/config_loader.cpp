...
std::string ConfigLoader::resolve_config_path() const {
  ...
  if (home) {
    std::string home_config =
        std::string(home) + "/.config/lazyrtui/config.yaml";
...
Config ConfigLoader::load() {
  ...
  try {
    ...
    if (node["keybindings"]) {
      ...
      if (kb["switch_focus"])
        config.keybindings.switch_focus = kb["switch_focus"].as<std::string>();
      if (kb["refresh"])
        config.keybindings.refresh = kb["refresh"].as<std::string>();
      if (kb["search"])
        config.keybindings.search = kb["search"].as<std::string>();
      if (kb["echo_topic"])
        config.keybindings.echo_topic = kb["echo_topic"].as<std::string>();
      if (kb["plot_topic"])
        config.keybindings.plot_topic = kb["plot_topic"].as<std::string>();
      if (kb["call_service"])
        config.keybindings.call_service = kb["call_service"].as<std::string>();
      if (kb["send_goal"])
        config.keybindings.send_goal = kb["send_goal"].as<std::string>();
      if (kb["help"])
        config.keybindings.help = kb["help"].as<std::string>();
      if (kb["quit"])
        config.keybindings.quit = kb["quit"].as<std::string>();
    }

    if (node["ui"]) {
      auto ui = node["ui"];
      if (ui["auto_refresh_interval_ms"])
        config.ui.auto_refresh_interval_ms =
            ui["auto_refresh_interval_ms"].as<int>();
      if (ui["mouse_support"])
        config.ui.mouse_support = ui["mouse_support"].as<bool>();
    }
    ...
    if (node["services"]) {
      for (auto it = node["services"].begin(); it != node["services"].end();
           ++it) {
        std::string service_name = it->first.as<std::string>();
        ServicePreset s_preset;
        if (it->second["params"]) {
          for (auto p_it = it->second["params"].begin();
               p_it != it->second["params"].end(); ++p_it) {
            s_preset.params[p_it->first.as<std::string>()] =
                p_it->second.as<std::string>();
          }
        }
