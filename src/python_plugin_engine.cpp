...
bool PythonPluginEngine::load_plugin_file(const std::string& file_path) {
    ...
    if (!PyObject_HasAttrString(module, "match") || !PyObject_HasAttrString(module, "render")) {
        std::cerr << "[lazyrtui] Plugin '" << file_path << "' missing required match() or render() functions." << std::endl;
        Py_DECREF(module);
        return false;
    }
    ...
    if (!updated) loaded_plugins_.push_back(info);

    return true;
}

std::string PythonPluginEngine::find_matching_plugin(const std::string& topic_name, const std::string& msg_type) {
    for (const auto& [mod_name, module] : plugin_modules_) {
        ...
        if (match_func && PyCallable_Check(match_func)) {
            PyObject* args = Py_BuildValue("(ss)", topic_name.c_str(), msg_type.c_str());
...
nlohmann::json PythonPluginEngine::render_message(const std::string& module_name,
                                                 const std::string& topic_name,
                                                 const std::string& json_body) {
    auto it = plugin_modules_.find(module_name);
    if (it == plugin_modules_.end()) {
        return {{"type", "text"}, {"content", "[Plugin " + module_name + " not loaded]"}, {"style", "red"}};
    }
    ...
    if (!render_func || !PyCallable_Check(render_func)) {
        ...
        return {{"type", "text"}, {"content", "[Plugin render() function missing]"}, {"style", "red"}};
    }
    ...
    if (!json_module) {
        ...
        return {{"type", "text"}, {"content", "[Python json module error]"}, {"style", "red"}};
    }

    PyObject* loads_func = PyObject_GetAttrString(json_module, "loads");
    PyObject* py_json_str = PyUnicode_FromString(json_body.c_str());
    PyObject* msg_dict = PyObject_CallFunctionObjArgs(loads_func, py_json_str, NULL);
    ...
    PyObject* render_result = PyObject_CallFunctionObjArgs(render_func, msg_dict, state_dict, NULL);
    Py_DECREF(msg_dict);
    Py_DECREF(render_func);
    ...
    if (!render_result) {
        std::string err = fetch_python_error();
        Py_DECREF(json_module);
        return {{"type", "text"}, {"content", "[Plugin Render Exception: " + err + "]"}, {"style", "red"}};
    }

    // Convert render_result to JSON string using json.dumps
    PyObject* dumps_func = PyObject_GetAttrString(json_module, "dumps");
    PyObject* res_json_obj = PyObject_CallFunctionObjArgs(dumps_func, render_result, NULL);
    ...
    if (!res_json_obj) {
        fetch_python_error();
        return {{"type", "text"}, {"content", "[Plugin return value not JSON serializable]"}, {"style", "red"}};
    }
    ...
    try {
        ...
        return {{"type", "text"}, {"content", "[Failed to parse plugin UI spec JSON]"}, {"style", "red"}};
    }
}
